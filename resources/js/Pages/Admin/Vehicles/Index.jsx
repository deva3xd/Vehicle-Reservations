  ...
  return (
    <PageLayout title="Vehicles">
      ...
      <div className="flex justify-between items-center my-2">
        ...
        <Link href={route('admin.vehicles.create')} className="px-6 py-2 rounded-lg cursor-pointer text-white text-className tracking-wider font-medium border-0 outline-0 outline-none bg-blue-700 hover:bg-blue-800 active:bg-blue-700">Add Data</Link>
      </div>
          ...
          <tbody className="whitespace-nowrap">
            {vehicles.map((v, i) => (
              <tr key={i}>
                ...
                <td className="p-4 text-[15px] text-slate-900 font-medium capitalize">
                  <div
                    className={`w-fit capitalize px-6 py-1 rounded-full text-white text-sm tracking-wider font-medium border-0 outline-0 outline-none ${v.status === 'tersedia' ? "bg-green-500" : v.status === 'dipakai' ? "bg-yellow-500" : 'bg-red-500'}`}
                  >
                    {v.status}
                  </div>
                </td>
                <td className="p-4">
                  <div className="flex items-center">
                    <Link href={route('admin.vehicles.edit', v.id)} className="mr-3 cursor-pointer">
                    ...
                    <button onClick={() => router.delete(route('admin.vehicles.destroy', v.id))} className="mr-3 cursor-pointer">
